...
function minimal(initialRate: number): TRefiningRule {
  return {
...
describe('fixed refining data', () => {
  it('maps document representative costs and materials', () => {
    expect(getRefiningRule('aegir', 'weapon', 10).gold).toBe(1296);
    expect(getRefiningRule('aegir', 'armor', 10).gold).toBe(776);
    expect(
      getRefiningRule('serka', 'weapon', 15).requiredMaterials.find(
        (item) => item.id === 'serka-leapstone',
      )?.quantity,
    ).toBe(25);
    expect(
      getRefiningRule('serka', 'weapon', 18).requiredMaterials.find(
  ...
  it('maps every Aegir book range to its exact effect and part-specific market item', () => {
    const expected = [
      [10, 1000, 'weapon-book-11-14'],
      [11, 1000, 'weapon-book-11-14'],
      [12, 500, 'weapon-book-11-14'],
      [13, 500, 'weapon-book-11-14'],
      [14, 400, 'weapon-book-15-18'],
      [15, 400, 'weapon-book-15-18'],
      [16, 300, 'weapon-book-15-18'],
      [17, 300, 'weapon-book-15-18'],
      [18, 300, 'weapon-book-19-20'],
      [19, 150, 'weapon-book-19-20'],
    ] as const;
    for (const [fromLevel, rateBonus, materialId] of expected) {
      const book = getRefiningRule('aegir', 'weapon', fromLevel).books.find(
        (item) => item.kind === 'normal',
      );
      expect(book).toMatchObject({ kind: 'normal', rateBonus, materialId });
    }
    expect(
      getRefiningRule('aegir', 'weapon', 18).books.find((item) => item.kind === 'enhanced'),
    ).toMatchObject({ rateBonus: 600, materialId: 'weapon-strong-book-19-20' });
    expect(
      getRefiningRule('aegir', 'armor', 14).books.find((item) => item.kind === 'normal'),
    ).toMatchObject({ materialId: 'armor-book-15-18' });
  });
  it('serializes all 58 documented material and currency rows without omissions', () => {
    ...
    const actual = equipment
      .flatMap(([equipmentGrade, equipmentType]) =>
        getAvailableRefiningLevels(equipmentGrade, equipmentType).map((fromLevel) =>
          getRefiningRule(equipmentGrade, equipmentType, fromLevel),
        ),
      )
      .map(
        (step) =>
          `${step.equipmentGrade}/${step.equipmentType}/${step.fromLevel}:${[...step.requiredMaterials.map((item) => item.quantity), step.gold, step.shilling].join(',')}`,
      )
      .join('|');
...
describe('Bellman plan', () => {
  ...
  it('charges a guaranteed success once with no optional material', () => {
    ...
    expect(plan.expectedShilling).toBe(1);
    expect(plan.recommendedWorstCase.shilling).toBe(1);
  ...
  it('treats held materials as 0G and supports 0G material prices', () => {
    ...
    const owned = calculateRefiningPlan(
      input(step, {
        prices,
        ownedMaterials: { 'aegir-leapstone': 2 },
      }),
    );
  ...
  it('allows intermediate breath counts and never combines book kinds', () => {
    const step = {
      ...
      books: [
        { kind: 'none' } as const,
        { kind: 'normal', rateBonus: 1, materialId: 'weapon-book-19-20' } as const,
        { kind: 'enhanced', rateBonus: 2, materialId: 'weapon-strong-book-19-20' } as const,
      ],
    };
  ...
  it('reports shilling separately, treats fate fragments as market material, and is deterministic', () => {
    const step = {
      ...minimal(5000),
      shilling: 10,
    ...
    expect(first.expectedShilling).toBeCloseTo(first.expectedAttempts * step.shilling);
    expect(first.recommendedWorstCase.shilling).toBe(
      first.recommendedWorstCase.attempts * step.shilling,
    );
  ...
  it('rejects negative or non-finite ownership and price inputs', () => {
    ...
    expect(() =>
      calculateRefiningPlan(
        input(minimal(10000), {
          ownedMaterials: { 'weapon-breath': -1 },
        }),
      ),
