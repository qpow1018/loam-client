...
import { TEST_MARKET_PRICES } from '../_define/refiningFixtures';
import { getRefiningStep, REFINING_STEPS } from '../_define/refiningSteps';
import type { TRefiningPlanInput, TRefiningStep } from '../_type/refining';
import { calculateRefiningPlan, getRefiningActions } from './calculateRefiningPlan';
...
describe('fixed refining data', () => {
  ...
  it('serializes all 58 documented material and currency rows without omissions', () => {
    ...
    const actual = REFINING_STEPS.map(
      (step) =>
        `${step.region}/${step.part}/${step.fromLevel}:${[...step.requiredMaterials.map((item) => item.quantity), step.gold, step.silver].join(',')}`,
    ).join('|');
