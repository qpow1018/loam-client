...
import styles from './arkPassiveNodeSection.module.scss';

const ARK_PASSIVE_CATEGORIES = ['진화', '깨달음', '도약'] as const;

type TArkPassiveCategory = (typeof ARK_PASSIVE_CATEGORIES)[number];

export default function ArkPassiveNodeSection(props: { arkPassive: TLostarkArkPassive }) {
  const { arkPassive } = props;

  if (arkPassive.nodes.length === 0) {
    return <p className={styles['empty-info']}>아크 패시브 상세 정보가 없습니다.</p>;
  }

  return (
    <div className={styles['ark-passive-node-section']}>
