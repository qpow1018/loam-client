...
export default function CreateCharacterModal(props: {
  ...
  const [unselectedNames, setUnselectedNames] = useState<Set<string>>(new Set());
  ...
  const {
    ...
    refetch,
  } = lostarkQuery.useGetSiblingCharacters(searchNickname);
  ...
  const isSubmitDisabled = unregisteredCharacters.every((character) =>
    unselectedNames.has(character.CharacterName),
  );
          ...
          {siblings.length > 0 && unregisteredCharacters.length === 0 ? (
            ...
            <div className={styles['character-grid']}>
              {unregisteredCharacters.map((character) => {
                const isSelected = !unselectedNames.has(character.CharacterName);
